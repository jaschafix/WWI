...
//genereed sequential array om id's te combineren
    $product_ids = array_column($_SESSION['shopping_cart'], 'StockItemID');
    if (!in_array(filter_input(INPUT_GET, 'StockItemID'), $product_ids)) {
    $_SESSION['shopping_cart'][$count] = array
      ...
      //match array key aan de id van het toe te voegen product
        for ($i = 0; $i < count($product_ids); $i++){
          if($product_ids[$i] == filter_input(INPUT_GET, 'StockItemID')) {
...
<div class="container">
  ...
  <div class="row box">
  <?php
  $stmt = $pdo->prepare("SELECT StockItemName, RecommendedRetailPrice, SearchDetails, QuantityOnHand, i.StockItemID FROM stockitems i LEFT JOIN stockitemholdings h ON i.StockItemID = h.StockItemID");
  $stmt->execute();
  $i = 1;
